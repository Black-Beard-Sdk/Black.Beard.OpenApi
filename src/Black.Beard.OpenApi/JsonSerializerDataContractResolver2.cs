...
namespace Bb
{
    ...
    internal class JsonSerializerDataContractResolver2 : ISerializerDataContractResolver
    {
        private readonly JsonSerializerOptions _serializerOptions;

        public JsonSerializerDataContractResolver2(JsonSerializerOptions serializerOptions)
        {
        ...
        public DataContract GetDataContractForType(Type type)
        {
            ...
            if (type.IsEnum)
            {
                ...
                Tuple<DataType, string> primitiveTypeAndFormat;
                var enumValues = type.GetEnumValues();

                bool hasConverter = HasConverter(type);
                if (hasConverter)
                {
                ...
                primitiveTypeAndFormat = PrimitiveTypesAndFormats[typeof(string)];
        ...
        private static bool HasConverter(Type type)
        {
            ...
            foreach (Attribute item in attributes)
                if (typeof(JsonConverterAttribute).IsAssignableFrom(item.GetType()))
                    return true;
